...
The Markov property asserts that these states are ``memoryless'':~in other words, the probability of transitioning from any state $i$ to any other state $j$ is independent of the history of the system before it was in $i$.
In order to obtain Markovian dynamics, a lag time must be chosen at which the transitions between states are analyzed. This lag time must be sufficiently short to resolve the dynamics of interest, but long enough such that the Markovian approximation is appropriate.

In order to create a Markov state model for a dynamical system, each data point in the time series is assigned to a state.
Given an appropriate lag time, every pairwise transition at that lag time is counted and stored in a counts matrix.
Then, the counts matrix is converted to a row-stochastic transition probability matrix, which is defined for the specified lag time.
To ensure that the transition probablity matrix has desirable mathematical properties, detailed balance is enforced when converting the counts matrix to the transition probability matrix.
...
The requirement of detailed balance indicates that we assume our system is in equilibrium.
We additionally require that the system is ergodic.

We can check whether a given transition probability matrix $T(\tau)$ is approximately Markovian using the Chapman-Kolmogorov (CK) test~\cite{msm-jhp}.
...
\noindent{}whe left-hand side of the equation corresponds to an MSM estimated at lag time $k\tau$, where $k$ is an integer larger than 1, whereas the right-hand side of the equation is our estimated MSM to the $k^\textrm{th}$ power.
...
The right eigenvectors, conventionally written as $\psi_i$, represent the flux into and out of the states of the MSM.
The corresponding left eigenvectors contain the same information weighted by the stationary distribution.
...
\begin{itemize}
	...
	\item Clustering. This is the step at which the state decomposition occurs. The features or TICs are grouped using a clustering algorithm such as $k$-means into a set of states.

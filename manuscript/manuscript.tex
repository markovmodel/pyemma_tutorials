...
\caption{The HMM transition matrix $\tilde{\mathbf{P}}(\tau)$ propagates the hidden state trajectory $\tilde{s}(t)$ (orange circles) and, at each time step $t$, the emission into the observable state $s(t)$ is governed by the emission probability $\bm{\chi}\left( s(t) \middle| \tilde{s}(t) \right)$.}
...
and gives rise to our observed data, e.g., the discretized trajectories.
...
we can similarly compute physical observables from directly.

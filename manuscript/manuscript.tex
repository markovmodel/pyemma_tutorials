...
\newcommand{\versionnumber}{0.3} % TODO: can we extract this number from a git tag?
...
Discrete jumps between the maxima can be observed by visualizing the transformation of the first trajectory into these ICs (Fig.~\ref{fig:io-to-ck}d).
 % time series visualization of these components from the first trajectory shows discrete jumps between these maxima (Fig.~\ref{fig:io-to-ck}d).
...
% In our case we have multiple gaps and therefore various options to choose from.
For this analysis, we chose 5 metastable states.
%, as it is the largest number that only includes processes which are well above the MSM lag time.
...
% the most probable state $\mathcal{S}_5$ acts as a transition state for transitions between states $\mathcal{S}_2$ and $\mathcal{S}_4$. 
...
While the present tutorial covers Markov State Modeling 101, we encourage the user to explore other, more recent extensions of the methodology.
...
Implementations of some of these methods are available or is under development in the package deeptime \url{github.com/markovmodel/deeptime}. 

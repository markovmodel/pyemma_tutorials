...
However, in case of a poor discretization or short trajectories,
it might be hard or even impossible to meet this requirement (as we will see in Notebook~07).
...
HMMs require the dynamics in some hidden (unobserved) state space to be Markovian.

To estimate an HMM, we need a spectral gap after the $m^\textrm{th}$ eigenvalue;
in practice, a timescale separation $t_m \geq 2t_{m+1}$ is sufficient~\cite{pyemma}.
...
$\tilde{\bm{\Pi}}=\left[ \tilde{\pi}_1,\dots,\tilde{\pi}_m \right]$ is a diagonal matrix of the $m$ hidden states' stationary probabilities,
$\tilde{\mathbf{P}}(\tau)$ is a (small) transition matrix between the $m$ hidden states at lag time $\tau$,
and $\bm{\chi}$ is an $m\times n$-dimensional row-stochastic matrix
where each row encodes the probabilities of the $n$ microstates to appear in the corresponding hidden state~\cite{noe-proj-hid-msm}.

An HMM estimation usually yields a small model where each hidden state is considered to be metastable and,
thus, the number of hidden states is a new hyper-parameter which needs to be chosen carefully.
Given that an HMM is a generalization of an MSM,
we can compute all physico-chemically relevant quantities available for MSMs, too.
...
we chose to adopt a sequential approach where only the hyper-pa\-ra\-me\-ters of the current stage are optimized.
...
(b)~can be enhanced if the system is prepared in the non\-equi\-lib\-ri\-um condition $\mathcal{S}_1$.

...
\begin{equation}
\label{eq:balance}
p_t(i)p(i,j) = p_t(j)p(j,i),
\end{equation}

\noindent{}where $p_t(i)$ represents the probability of being in state $i$ at time $t$, and $p(i,j)$ is the probability of transitioning from state $i$ to $j$ at the next time step.
...
The right eigenvectors, conventionally written as $\psi_i$, each represent a dynamical process (for $i>1$), and the coefficients of the eigenvectors represent the flux into and out of the MSM states that characterizes that process.

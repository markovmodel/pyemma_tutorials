...
\item notebook -- The Jupyter notebook tool used for running the tutorials~\cite{jupyter}, along with extension packages jupyter_contrib_nbextensions and nbexamples
...
To perform this test, we first choose a number of metatstable states appropriate for our system. Then, PyEMMA automatically estimates additional MSM transition matrices at lag times $k \tau$ and compares these estimates to the $k^\textrm{th}$ power of the original transition matrix $T(\tau)$.

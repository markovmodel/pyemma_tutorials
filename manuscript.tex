...
\begin{abstract}
This tutorial provides an introduction to the PyEMMA project, a Python application program interface for constructing Markov state models and related analyses from biomolecular simulation data.
...
As it is unknown which molecular features are best at representing the slow dynamics of the pentapeptide, we perform a broad systematic analysis using the variational approach for Markov processes (VAMP)~\cite{vamp-preprint}.
We utilize the VAMP-2 score, which maximizes the kinetic variance of the estimated dynamical operator~\cite{kinetic-maps}.
...
To perform this test, PyEMMA automatically estimates a new MSM transition matrix at lag time $k \tau$ and propagates the original transition matrix by the $k^\textrm{th}$ power.
As we have resolved three slow timescales in the ITS test, we assume that four metastable states are present in the data. The subsequent CK test shows that indeed our model predicts the kinetic behavior on longer timescales accurately (Fig.~\ref{fig:io-to-ck}f).
...
As an example TPT analysis, we compute the flux between macrostates $\mathcal{S}_2$ and $\mathcal{S}_3$ (Fig.~\ref{fig:msm-analysis}d).
...
Finally, we sample $200$ super-imposed molecular conformations from each of the $4$ macrostates (Fig.~\ref{fig:msm-analysis}g) and render them using VMD~\cite{humphrey1996vmd}.
